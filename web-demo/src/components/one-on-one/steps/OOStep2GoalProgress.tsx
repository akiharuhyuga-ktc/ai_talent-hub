...
export function OOStep2GoalProgress({ goalProgress, onNext, onBack }: Props) {
  ...
  const updateProgress = (index: number, field: keyof GoalProgressEntry, value: string) => {
  ...
  return (
    <div>
      ...
      <p className="text-xl text-gray-500 mb-8">各目標の進捗状況を入力してください。</p>
      ...
      {hasGoals ? (
        <div className="space-y-6 mb-10">
          {progress.map((entry, i) => (
            <div key={i} className="bg-gray-50 border border-gray-200 rounded-lg p-6">
              <div className="mb-4">
                <h3 className="text-xl font-medium text-gray-800 mb-2">{entry.goalLabel}</h3>
                <p className="text-lg text-gray-600 mb-2">{entry.goalText}</p>
                {entry.achievedState && (
                  <p className="text-lg text-gray-500">
                    <span className="font-medium">達成状態：</span>{entry.achievedState}
                  </p>
                )}
                {entry.milestone && (
                  <p className="text-lg text-gray-500">
                    <span className="font-medium">マイルストーン：</span>{entry.milestone}
                  </p>
                )}
              </div>
        ...
        <div className="mb-10">
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-5 mb-6">
            <p className="text-xl text-amber-700">目標が未設定です。自由記述で進捗を記録してください。</p>
          </div>
          <div>
            <label className="block text-xl font-medium text-gray-700 mb-2">
