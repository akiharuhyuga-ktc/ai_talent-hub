...
function SkillRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="flex gap-5 py-4 border-b border-gray-100 last:border-0">
      <dt className="text-xl font-medium text-gray-400 w-32 shrink-0">{label}</dt>
      <dd className={`text-xl leading-relaxed ${isEmpty ? 'text-gray-300' : 'text-gray-800'}`}>
        {isEmpty ? '—' : value}
      </dd>
    </div>
  )
    ...
    <div className="space-y-8">
      ...
      <section className="bg-gradient-to-r from-brand-50 to-purple-50 rounded-radius-2xl p-8 border border-brand-100">
        <div className="flex items-center gap-7">
          <div className={`w-24 h-24 rounded-radius-2xl bg-gradient-to-br ${gradient} flex items-center justify-center shrink-0 shadow-glow`}>
            <span className="text-white font-bold text-5xl">{member.name.slice(0, 1)}</span>
          </div>
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xl text-gray-400 mt-1">{member.team} ｜ 入社：{member.joinedAt}</p>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-7">
        ...
        <div className="space-y-7">
          {/* Skills */}
          <section>
            <h3 className="text-2xl font-semibold text-gray-800 mb-3">スキル・経験</h3>
          ...
          {(member.expectedRole.current || member.expectedRole.longTerm) && (
            <section>
              <h3 className="text-2xl font-semibold text-gray-800 mb-3">期待する役割</h3>
              <div className="space-y-3">
                {member.expectedRole.current && (
                  <div className="bg-brand-50 rounded-radius-xl p-5 border border-brand-100">
                    <p className="text-lg font-semibold text-brand-600 uppercase tracking-wide mb-2">現在の期待役割</p>
                ...
                {member.expectedRole.longTerm && (
                  <div className="bg-purple-50 rounded-radius-xl p-5 border border-purple-100">
                    <p className="text-lg font-semibold text-purple-600 uppercase tracking-wide mb-2">中長期キャリア方向性</p>
                    <p className="text-xl text-gray-800 whitespace-pre-line leading-relaxed">
                      {member.expectedRole.longTerm}
                    </p>
                  </div>
        ...
        <div>
          {member.projects.length > 0 ? (
            <section>
              <h3 className="text-2xl font-semibold text-gray-800 mb-3">担当プロジェクト（2026年4〜6月）</h3>
              <div className="space-y-4">
                {member.projects.map(proj => (
                  <div key={proj.name} className="bg-white rounded-radius-xl shadow-card p-5">
                    <div className="flex justify-between items-start mb-3">
                      <span className="text-2xl font-semibold text-gray-900 leading-snug">{proj.name}</span>
                      <span className="text-3xl font-bold text-brand-600 ml-4 shrink-0">{proj.avgPct}%</span>
                    </div>
