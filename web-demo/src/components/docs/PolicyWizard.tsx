...
export interface PolicyWizardState {
  ...
  continuousThemes: { envChanges: string; techChanges: string; focusThemes: string }
  // Initial flow (Step2B, Step3B)
  currentState: { teamInfo: string; techDomains: string; challenges: string; strengths: string; mission: string; themes: string }
  upperPolicy: string
  // Common (Step4+)
...
type Action =
  | { type: 'SET_FLOW_MODE'; payload: { targetYear: number; flowMode: PolicyFlowMode; baseContent: string } }
  | { type: 'SET_REVIEW'; payload: { whatWorked: string; whatDidntWork: string; leftBehind: string } }
  | { type: 'SET_CURRENT_STATE'; payload: { teamInfo: string; techDomains: string; challenges: string; strengths: string; mission: string; themes: string } }
  | { type: 'SET_CONTINUOUS_THEMES'; payload: { envChanges: string; techChanges: string; focusThemes: string } }
...
const initialState: PolicyWizardState = {
  ...
  currentState: { teamInfo: '', techDomains: '', challenges: '', strengths: '', mission: '', themes: '' },
  upperPolicy: '',
  direction: null,
  aiDraft: null,
  currentDraft: '',
...
function reducer(state: PolicyWizardState, action: Action): PolicyWizardState {
  switch (action.type) {
    ...
    case 'SET_CONTINUOUS_THEMES':
      return { ...state, continuousThemes: action.payload, currentStep: 4, direction: null, aiDraft: null }
    case 'SET_UPPER_POLICY':
      return { ...state, upperPolicy: action.payload, currentStep: 4, direction: null, aiDraft: null }
    case 'SET_DIRECTION':
      return { ...state, direction: action.payload, currentStep: 5, aiDraft: null }
    case 'SET_AI_DRAFT':
      return { ...state, aiDraft: action.payload, currentDraft: action.payload, currentStep: 6 }
    case 'UPDATE_DRAFT':
    ...
    case 'PREV_STEP': {
      ...
      if (state.currentStep >= 4) {
        return { ...state, currentStep: prevStep, direction: null, aiDraft: null }
      }
...
export function PolicyWizard({ availableYears, criteria: _criteria, guidelines: _guidelines, onClose }: PolicyWizardProps) {
    ...
    switch (state.currentStep) {
        ...
        return (
          <PolicyStep1Year
            ...
            onNext={(targetYear, flowMode, baseContent) =>
              dispatch({ type: 'SET_FLOW_MODE', payload: { targetYear, flowMode, baseContent } })
            }
          />
        )
      case 2:
        if (state.flowMode === 'continuous') {
          ...
          return (
            <PolicyStep2BCurrentState
              onNext={data => dispatch({ type: 'SET_CURRENT_STATE', payload: data })}
              onBack={() => dispatch({ type: 'PREV_STEP' })}
            />
          )
        }
      case 3:
        if (state.flowMode === 'continuous') {
          return (
            <PolicyStep3AThemes
              onNext={data => dispatch({ type: 'SET_CONTINUOUS_THEMES', payload: data })}
              onBack={() => dispatch({ type: 'PREV_STEP' })}
            />
          )
        } else {
          return (
            <PolicyStep3BUpperPolicy
              onNext={policy => dispatch({ type: 'SET_UPPER_POLICY', payload: policy })}
              onBack={() => dispatch({ type: 'PREV_STEP' })}
            />
          )
        }
      case 4:
        return (
          <PolicyStep4Direction
            state={state}
            onConfirm={direction => dispatch({ type: 'SET_DIRECTION', payload: direction })}
            onBack={() => dispatch({ type: 'PREV_STEP' })}
          />
        )
      case 5:
      ...
      case 6:
        return (
          <PolicyStep6Refine
            state={state}
            onContentUpdate={content => dispatch({ type: 'UPDATE_DRAFT', payload: content })}
            onNext={finalContent => dispatch({ type: 'CONFIRM_DRAFT', payload: finalContent })}
            onBack={() => dispatch({ type: 'PREV_STEP' })}
          />
        )
      case 7:
  ...
  const headerTitle = state.targetYear > 0
      ...
      {showExitConfirm && (
        <div className="fixed inset-0 z-[60] bg-black/40 flex items-center justify-center">
          <div className="bg-white rounded-xl shadow-xl p-8 max-w-md w-full mx-4">
            <h3 className="text-xl font-bold text-gray-800 mb-3">作成を中止しますか？</h3>
