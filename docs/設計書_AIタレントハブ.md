...
> 文書バージョン：2.4
> 作成日：2026-03-21
> 最終更新：2026-04-08（frontend を唯一の現行アプリとして再整理。ルートの起動用 package.json は撤去）
...
├── frontend/                   ← 現行の Next.js Webアプリケーション
│   ├── package.json            ← アプリ本体の依存関係・スクリプト定義
│   ├── next.config.mjs
│   ├── tailwind.config.ts
│   ├── tsconfig.json
│   ├── postcss.config.mjs
...
| ファイルパス | 役割 |
|------------|------|
| `frontend/package.json` | 現行アプリ本体の依存関係・スクリプト定義 |
| `frontend/next.config.mjs` | Next.js 設定（空） |
| `frontend/tsconfig.json` | TypeScript 設定。`@/*` → `./src/*` パスエイリアス |
| `frontend/tailwind.config.ts` | Tailwind CSS 設定。indigo カラーをカスタムパレット（#19708C 系ティール）に上書き。typography プラグイン有効 |
| `frontend/postcss.config.mjs` | PostCSS 設定 |
| `.gitignore` | data/, .env, Office ファイルを除外 |
| `frontend/.gitignore` | Next.js 標準の除外設定 |

### 9.2 ページ（`frontend/src/app/`）
...
### 9.3 APIルート（`frontend/src/app/api/`）
...
### 9.4 コンポーネント（`frontend/src/components/`）
...
| ファイルパス | 種類 | 役割 |
|------------|------|------|
| `frontend/src/components/one-on-one/OneOnOneWizard.tsx` | Client Component | 1on1ウィザード本体（useReducer + 5ステップ描画） |
| `frontend/src/components/one-on-one/OneOnOneStepper.tsx` | Client Component | ステッパーUI（5ステップの進捗表示） |
| `frontend/src/components/one-on-one/steps/OOStep1ActionReview.tsx` | Client Component | Step1: 前回アクションアイテム振り返り |
| `frontend/src/components/one-on-one/steps/OOStep2GoalProgress.tsx` | Client Component | Step2: 目標進捗確認 |
| `frontend/src/components/one-on-one/steps/OOStep3Condition.tsx` | Client Component | Step3: コンディション確認 |
| `frontend/src/components/one-on-one/steps/OOStep4Hearing.tsx` | Client Component | Step4: AIヒアリング質問 + メモ |
| `frontend/src/components/one-on-one/steps/OOStep5NextActions.tsx` | Client Component | Step5: ネクストアクション設定 |
| `frontend/src/components/one-on-one/steps/OOCompletionScreen.tsx` | Client Component | 完了画面 |
| `frontend/src/app/api/members/[name]/one-on-one/route.ts` | API Route | 1on1記録保存 |
| `frontend/src/app/api/members/[name]/one-on-one/questions/route.ts` | API Route | AIヒアリング質問生成 |
| `frontend/src/app/api/members/[name]/one-on-one/summary/route.ts` | API Route | AI引き継ぎサマリー生成 |
| `frontend/src/lib/prompts/one-on-one-questions.ts` | Prompt Builder | ヒアリング質問用プロンプト（buildOneOnOneQuestionsSystemPrompt, buildOneOnOneQuestionsUserMessage） |
| `frontend/src/lib/prompts/one-on-one-summary.ts` | Prompt Builder | 引き継ぎサマリー用プロンプト（buildOneOnOneSummarySystemPrompt, buildOneOnOneSummaryUserMessage） |
| `frontend/src/lib/parsers/one-on-one.ts` | Parser | 1on1記録パーサー（parseOneOnOneRecord: アクションアイテム・コンディション・引き継ぎサマリー抽出） |
| `frontend/src/lib/parsers/goal-progress.ts` | Parser | 目標分解パーサー（parseGoalEntries: 目標Markdownを個別エントリに分解） |
| `frontend/src/lib/utils/timeline-warning.ts` | Utility | タイムライン警告計算（getTimelineWarning: 中間確認期限の残日数計算） |
| `frontend/src/lib/utils/one-on-one-markdown.ts` | Utility | 1on1記録Markdown組み立て（buildOneOnOneMarkdown: ウィザード状態からMarkdown文字列を生成） |

#### 10.6.2 修正が必要な既存ファイル

| ファイルパス | 修正内容 |
|------------|---------|
| `frontend/src/lib/types.ts` | `ConditionScore`, `ActionItemReview`, `GoalProgressEntry`, `HearingQuestion`, `ActionItem`, `OneOnOneWizardState`, `OneOnOneWizardContextData` の7型を追加 |
| `frontend/src/components/member/MemberDetailClient.tsx` | `oneOnOneWizardOpen` state を追加。`OneOnOneWizard` コンポーネントのインポートと描画。`OneOnOneTab` に `onStartWizard` props を渡す |
| `frontend/src/components/member/OneOnOneTab.tsx` | 「1on1ウィザードを開始」ボタンを追加。`onStartWizard` props を受け取る |
| `frontend/src/app/members/[name]/page.tsx` | `oneOnOneWizardContext` を構築して `MemberDetailClient` に渡す。前回の1on1記録を取得してコンテキストに含める |
| `frontend/src/lib/fs/members.ts` | `getLatestOneOnOne(memberName)` 関数を追加（最新の1on1記録を取得） |
...
| ファイルパス | 種類 | 役割 |
|------------|------|------|
| `frontend/src/components/evaluation/ReviewWizard.tsx` | Client Component | 評価ウィザード本体（useReducer + 4ステップ描画） |
| `frontend/src/components/evaluation/EvaluationStepper.tsx` | Client Component | ステッパーUI（4ステップの進捗表示） |
| `frontend/src/components/evaluation/steps/EvalStep1Materials.tsx` | Client Component | Step1: 評価素材の自動収集 + 自己評価入力 |
| `frontend/src/components/evaluation/steps/EvalStep2AIDraft.tsx` | Client Component | Step2: AI評価ドラフト表示 |
| `frontend/src/components/evaluation/steps/EvalStep3Review.tsx` | Client Component | Step3: マネージャー確認・修正 |
| `frontend/src/components/evaluation/steps/EvalStep4Comment.tsx` | Client Component | Step4: AI評価者コメント生成 + 編集 |
| `frontend/src/components/evaluation/EvalCompletionScreen.tsx` | Client Component | 完了画面（評価サマリー + 次期目標誘導） |
| `frontend/src/app/api/members/[name]/reviews/route.ts` | API Route | 評価保存 |
| `frontend/src/app/api/members/[name]/reviews/draft/route.ts` | API Route | AI評価ドラフト生成 |
| `frontend/src/app/api/members/[name]/reviews/comment/route.ts` | API Route | AI評価者コメント生成 |
| `frontend/src/lib/prompts/review-draft.ts` | Prompt Builder | 評価ドラフト用プロンプト（buildReviewDraftSystemPrompt, buildReviewDraftUserMessage） |
| `frontend/src/lib/prompts/review-comment.ts` | Prompt Builder | 評価者コメント用プロンプト（buildReviewCommentSystemPrompt, buildReviewCommentUserMessage） |
| `frontend/src/lib/utils/review-markdown.ts` | Utility | 評価Markdown組み立て（buildReviewMarkdown: ウィザード状態からMarkdown文字列を生成） |
| `frontend/src/lib/utils/evaluation-context.ts` | Utility | 評価コンテキスト構築（buildEvaluationContext: 1on1記録から進捗トレンド・コンディション推移等を集約） |
| `frontend/src/lib/parsers/one-on-one.ts` | Parser（関数追加） | `parseGoalProgress()` 関数を新設（1on1記録の `## 目標進捗` セクションから `{ goalLabel, status, progressComment }` を抽出） |

#### 11.6.2 修正が必要な既存ファイル

| ファイルパス | 修正内容 |
|------------|---------|
| `frontend/src/lib/types.ts` | `EvaluationGrade`, `SelfEvaluation`, `ManagerSupplementary`, `GoalEvaluation`, `EvaluationDraft`, `EvaluationWizardState`, `EvaluationWizardContextData` の7型を追加 |
| `frontend/src/components/member/MemberDetailClient.tsx` | `reviewWizardOpen` state を追加。`ReviewWizard` コンポーネントのインポートと描画。`ReviewsTab` に `onStartWizard` props を渡す |
| `frontend/src/components/member/ReviewsTab.tsx` | 「評価ウィザードを開始」ボタンを追加。`onStartWizard` props を受け取る。パスワード解除後に表示 |
| `frontend/src/app/members/[name]/page.tsx` | `evaluationWizardContext` を構築して `MemberDetailClient` に渡す。全1on1記録のパースと集約。前期評価の取得 |
| `frontend/src/lib/fs/members.ts` | `getLatestReview(memberName)` 関数を追加（最新の評価記録を取得） |
| `frontend/src/lib/parsers/goals-entries.ts` | 評価ウィザードからも参照されるため、エクスポートの確認のみ（変更不要の可能性あり） |
| `frontend/src/lib/types.ts` | `WizardContextData` に `previousReview: ReviewData | null` を追加（評価→次期目標連携用） |
| `frontend/src/app/members/[name]/page.tsx` | 最新の評価ファイルを読み込み、`WizardContextData` のコンテキストに含める処理を追加 |
| `frontend/src/components/goals/GoalWizard.tsx` | Step4（前期実績）で `previousReview` が存在する場合に `previousGoals`・`achievementLevel`・`reasonIfNotAchieved` を自動入力 |
| `frontend/src/components/member/ReviewsTab.tsx` | `evalColorMap` に D評価の色定義を追加：`D: { bg: 'bg-red-100', text: 'text-red-700', border: 'border-red-400' }` |
...
| ファイルパス | 削除内容 | 状態 |
|------------|---------|------|
| `frontend/src/lib/mock/responses.ts` | ファイル全体を削除。`getMockResponse()` 関数と `MOCK_QA_PAIRS` 定数、`FALLBACK_RESPONSE` 定数を完全に除去 | **削除済み（v1.6）** |
...
| `frontend/src/app/api/demo-mode/route.ts` | API Route | デモモード状態の取得・切替 |

#### 13.5.2 修正が必要な既存ファイル

| ファイルパス | 修正内容 |
|------------|---------|
| `frontend/src/lib/fs/paths.ts` | `DEMO_MEMBERS_DIR` 定数および `getMembersDir()` 関数を追加。デモモード状態に応じてデータディレクトリを切り替え |
| `frontend/src/lib/fs/members.ts` | メンバー一覧・詳細取得で `getMembersDir()` を使用するよう変更 |
| `frontend/src/components/layout/NavBar.tsx` | デモモードトグルスイッチ + 「デモ版」バッジ + アンバーバナーを追加 |
| `frontend/src/app/api/members/[name]/goals/route.ts` | `getMembersDir()` 経由でパスを解決するよう変更 |
| `frontend/src/app/api/members/[name]/one-on-one/route.ts` | 同上 |
| `frontend/src/app/api/members/[name]/reviews/route.ts` | 同上 |
...
| ファイルパス | 種類 | 役割 |
|------------|------|------|
| `frontend/src/components/policy/PolicyWizard.tsx` | Client Component | 組織方針ウィザード本体（useReducer + 7ステップ分岐描画） |
| `frontend/src/components/policy/PolicyStepper.tsx` | Client Component | ステッパーUI（7ステップの進捗表示、フローモードに応じたラベル切替） |
| `frontend/src/components/policy/steps/PolicyStep1Year.tsx` | Client Component | Step1: 年度選択 + フローモード検出 |
| `frontend/src/components/policy/steps/PolicyStep2AReview.tsx` | Client Component | Step2A: 前年度振り返り（継続モード） |
| `frontend/src/components/policy/steps/PolicyStep2BCurrentState.tsx` | Client Component | Step2B: 現在の組織状態（初回モード） |
| `frontend/src/components/policy/steps/PolicyStep3AThemes.tsx` | Client Component | Step3A: 来期テーマ・環境変化（継続モード） |
| `frontend/src/components/policy/steps/PolicyStep3BUpperPolicy.tsx` | Client Component | Step3B: 上位組織方針（初回モード） |
| `frontend/src/components/policy/steps/PolicyStep4Direction.tsx` | Client Component | Step4: AI方向性提案（共通） |
| `frontend/src/components/policy/steps/PolicyStep5Draft.tsx` | Client Component | Step5: AIドラフト表示（共通） |
| `frontend/src/components/policy/steps/PolicyStep6Refine.tsx` | Client Component | Step6: 壁打ち・精緻化（共通） |
| `frontend/src/components/policy/steps/PolicyStep7Confirm.tsx` | Client Component | Step7: 確認・保存（共通、フローモードに応じた差分表示切替） |
| `frontend/src/components/policy/steps/PolicyStepComplete.tsx` | Client Component | Step8: 完了画面（保存成功後の確認画面） |
...
| ファイルパス | 種類 | 役割 |
|------------|------|------|
| `frontend/src/lib/utils/period.ts` | Utility | 期間管理基盤（Period型、getActivePeriod、formatPeriodLabel、sortPeriods、parsePeriodFromFilename、getFiscalYear） |
| `frontend/src/components/policy/PolicyWizard.tsx` | Client Component | 組織方針ウィザード本体（useReducer + 7ステップ分岐描画） |
| `frontend/src/components/policy/PolicyStepper.tsx` | Client Component | ステッパーUI（7ステップの進捗表示、フローモードに応じたラベル切替） |
| `frontend/src/components/policy/steps/PolicyStep1Year.tsx` | Client Component | Step1: 年度選択 + フローモード検出 |
| `frontend/src/components/policy/steps/PolicyStep2AReview.tsx` | Client Component | Step2A: 前年度振り返り（継続モード） |
...
| `frontend/src/components/policy/steps/PolicyStep7Confirm.tsx` | Client Component | Step7: 確認・保存（共通、フローモードに応じた差分表示切替） |
| `frontend/src/components/policy/steps/PolicyStepComplete.tsx` | Client Component | Step8: 完了画面（保存成功後の確認画面） |
| `frontend/src/app/api/docs/policy/direction/route.ts` | API Route | AI組織方針の方向性・骨子提案（v1.9新設） |
| `frontend/src/app/api/docs/policy/refine/route.ts` | API Route | AI組織方針リファイン |
| `frontend/src/app/api/docs/policy/save/route.ts` | API Route | 組織方針保存 |
| `frontend/src/lib/prompts/policy-direction.ts` | Prompt Builder | 組織方針方向性提案用プロンプト（buildPolicyDirectionSystemPrompt, buildPolicyDirectionUserMessage）（v1.9新設） |
| `frontend/src/lib/prompts/policy-draft.ts` | Prompt Builder | 組織方針ドラフト用プロンプト（buildPolicyDraftSystemPrompt, buildPolicyDraftUserMessage）（v1.9でモード別プロンプトに変更） |
| `frontend/src/lib/prompts/policy-refine.ts` | Prompt Builder | 組織方針リファイン用プロンプト（buildPolicyRefineSystemPrompt, buildPolicyRefineUserMessage） |
...
| 廃止ファイルパス | 置換先 |
|----------------|--------|
| `frontend/src/components/policy/steps/PolicyStep1Input.tsx` | `PolicyStep1Year.tsx` + `PolicyStep2AReview.tsx` / `PolicyStep2BCurrentState.tsx` + `PolicyStep3AThemes.tsx` / `PolicyStep3BUpperPolicy.tsx` に分割 |
| `frontend/src/components/policy/steps/PolicyStep2Draft.tsx` | `PolicyStep5Draft.tsx` に置換 |
| `frontend/src/components/policy/steps/PolicyStep3Refine.tsx` | `PolicyStep6Refine.tsx` に置換 |
| `frontend/src/components/policy/steps/PolicyStep4Preview.tsx` | `PolicyStep7Confirm.tsx` に置換 |

#### 14.8.3 修正が必要な既存ファイル

| ファイルパス | 修正内容 |
|------------|---------|
| `frontend/src/lib/types.ts` | `MemberDetail` に `goalsByPeriod`, `activePeriod` を追加。`GoalWizardState` に `targetPeriod` を追加。`PolicyWizardState` 型を v1.9 の7ステップ分岐フロー仕様に新設（`PolicyFlowMode` 型、`review`, `currentState`, `continuousThemes`, `upperPolicy`, `direction` 等のフィールドを含む）。`EvaluationWizardState` に `availablePeriods` を追加 |
| `frontend/src/lib/fs/members.ts` | `getAllGoalPeriods()`, `getGoalsByPeriod()` を新設。`getMemberDetail()` を複数期間対応に改修 |
| `frontend/src/lib/fs/shared-docs.ts` | `loadOrgPolicy()`, `getOrgPolicyYears()` を新設。`loadSharedDocs()` を年度対応に改修。戻り値キーを `policy` → `orgPolicy` に変更 |
| `frontend/src/lib/fs/paths.ts` | `SHARED_DOCS.policy` は後方互換のため残存。`SHARED_DIR` のエクスポートを確認 |
| `frontend/src/app/api/docs/route.ts` | `year` クエリパラメータ対応。レスポンスキーを `policy` → `orgPolicy` に変更。`availableYears` を追加 |
| `frontend/src/app/api/members/[name]/goals/route.ts` | `period` パラメータを必須化。対象期間ラベルの動的生成。バリデーション追加 |
| `frontend/src/components/member/GoalsTab.tsx` | 期間セレクターUI追加。`goalsByPeriod`, `activePeriod` props を受け取る |
| `frontend/src/components/member/MemberDetailClient.tsx` | `GoalsTab` への `goalsByPeriod`, `activePeriod` props 追加。`policyWizardOpen` state は不要（/docs ページで管理） |
| `frontend/src/components/docs/DocsTabs.tsx` | タブラベル「部方針」→「組織方針」。年度セレクター追加。`orgPolicy` プロパティ対応。PolicyWizard 起動ボタン追加 |
| `frontend/src/app/docs/page.tsx` | `loadSharedDocs()` + `getOrgPolicyYears()` 経由に書き換え、`availableYears` を `DocsTabs` に渡す |
| `frontend/src/app/members/[name]/page.tsx` | `loadSharedDocs()` の戻り値変更に対応。`goalsByPeriod`, `activePeriod` の構築 |
| `frontend/src/components/layout/NavBar.tsx` | 「部方針」→「組織方針」リンクラベル変更 |
| `frontend/src/lib/prompts/diagnosis.ts` | ユーザーメッセージ内「部方針」→「組織方針」 |
| `frontend/src/lib/prompts/goal-generation.ts` | ユーザーメッセージ内「部方針」→「組織方針」 |
| `frontend/src/lib/prompts/one-on-one-questions.ts` | ユーザーメッセージ内「部方針」→「組織方針」 |
| `frontend/src/lib/prompts/one-on-one-summary.ts` | コンテキスト変数名 `departmentPolicy` → `orgPolicy` |
| `frontend/src/lib/prompts/review-draft.ts` | ユーザーメッセージ内「部方針」→「組織方針」、変数名 `departmentPolicy` → `orgPolicy` |
| `frontend/src/components/goals/GoalWizard.tsx` | `targetPeriod` の初期化・保存時送信。ヘッダーに期間ラベル表示 |
| `frontend/src/components/goals/steps/Step1AutoLoad.tsx` | 「部方針」→「組織方針」表示ラベル変更 |
| `frontend/src/app/api/members/[name]/reviews/draft/route.ts` | リクエストボディに `policyYear` パラメータを追加し、`loadOrgPolicy(policyYear)` で対応年度の方針を参照する |
| `frontend/src/app/api/members/[name]/reviews/route.ts` | `period \|\| '2026-h1'` のハードコードを `period \|\| getActivePeriod()` に変更 |
| `frontend/src/app/api/chat/route.ts` | `loadSharedDocs()` の戻り値型変更に追従（`shared.guidelines` のみ使用のため実質影響なし） |
| `frontend/src/app/api/docs/policy/draft/route.ts` | v1.9: `mode`, `direction` パラメータを追加。モード別のシステムプロンプト・ユーザーメッセージ構築に変更 |
...
| ファイルパス | 種類 | 役割 |
|------------|------|------|
| `frontend/src/app/api/team/matrix/route.ts` | API Route | チームマトリクスデータ取得（GET） |
| `frontend/src/app/team/page.tsx` | Server Component | チームマトリクスビューページ。`TeamMatrixView` を描画 |
| `frontend/src/components/dashboard/TeamMatrixView.tsx` | Client Component | チームマトリクスビューのコンテナ。期間セレクター、チームフィルターピル、サマリーチップ、`TeamMatrixTable` を内包 |
| `frontend/src/components/dashboard/TeamMatrixTable.tsx` | Client Component | マトリクステーブル本体。メンバー行 × ステータス列のテーブル描画。行クリック遷移 |
| `frontend/src/components/dashboard/MatrixCell.tsx` | Server Component | マトリクスセル。ステータス（done / missing / future）に応じた ○/×/－ の描画 |

### 16.6 既存ファイル変更

| ファイルパス | 修正内容 |
|------------|---------|
| `frontend/src/lib/types.ts` | `MemberPeriodStatus`、`TeamPeriodMatrix` の2型を追加 |
| `frontend/src/lib/fs/members.ts` | `getMemberPeriodStatus()`、`getOneOnOneMonthsForPeriod()`、`getTeamPeriodMatrix()`、`getAvailablePeriods()` の4関数を追加 |
| `frontend/src/components/layout/NavBar.tsx` | `navItems` 配列に `{ href: '/team', label: 'チームマトリクス' }` を追加 |
