...
今期の目標設計の出発点となる「診断サマリー」をマークダウン形式で生成してください。
...
- メンバーが読んで「よし、この方向で目標を作ろう」と前向きになれるトーンで書く
- 「〜できていない」「〜が不足している」「〜がない」などの欠如表現を使わない
- 「まだ〜していない」「〜ができていない段階」などの評価表現を避ける
- 現状の批評ではなく、「この人がこれから何をしようとしているか」の文脈で書く`;
