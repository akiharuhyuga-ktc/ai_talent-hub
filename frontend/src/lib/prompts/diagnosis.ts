...
以下の情報をもとに、���のメンバー���今期目標設計の出発点となる「成長ナラティブ」を作成してください。

【出力フォーマット】

今期の成長テーマ：
　[この人が今期に踏��出せる変化・チャレンジを1〜2文で]
...
export function buildDiagnosisUserMessage(
	...
	const parts = [
		...
		"",
		"## マネージャーからの期待",
		`- このメンバ��への期待：${params.managerInput.expectations}`,
	...
	if (params.previousPeriod?.previousGoals) {
		parts.push(
			"",
			"## 前期実績",
