...
function buildReviewMarkdown(
	...
	if (!state.confirmedDraft) return "";
	...
	if (changes.length > 0) {
		...
		for (const ge of changes) {
			const aiGrade =
				state.aiDraft?.goalEvaluations.find((a) => a.goalLabel === ge.goalLabel)
					?.grade || "?";
			lines.push(
				`- ${ge.goalLabel}：AI評価 ${aiGrade} → 確定 ${ge.grade}（理由：${ge.changeReason}）`,
			);
		}
		lines.push("");
	}
...
export function EvalCompletionScreen({ state, context, onClose }: Props) {
	...
	if (!state.confirmedDraft) return null;
	const draft = state.confirmedDraft;
	...
	const overallColors = draft.overallGrade
		? gradeColorMap[draft.overallGrade] || gradeColorMap.B
		: gradeColorMap.B;
			...
			<div className="mb-8">
				<div className="bg-white border border-gray-200 rounded-lg p-6">
					<div className="flex items-center gap-4 mb-6">
						<span
							className={`inline-flex items-center justify-center w-20 h-20 rounded-xl text-4xl font-bold border-2 ${overallColors.bg} ${overallColors.text} ${overallColors.border}`}
						>
							{draft.overallGrade}
						</span>
						<div>
							<p className="text-xl font-medium text-gray-800">
								{context.memberName}さんの総合評価
							</p>
					...
					<div className="space-y-3">
						{draft.goalEvaluations.map((ge) => {
							const colors = ge.grade
								? gradeColorMap[ge.grade] || gradeColorMap.B
								: gradeColorMap.B;
							return (
								<div key={ge.goalLabel} className="flex items-center gap-3">
									<span
										className={`inline-flex items-center justify-center w-10 h-10 rounded-lg text-lg font-bold border-2 ${colors.bg} ${colors.text} ${colors.border}`}
									>
										{ge.grade}
									</span>
