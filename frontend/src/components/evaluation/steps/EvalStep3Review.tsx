...
export function EvalStep3Review({ state, onConfirm, onBack }: Props) {
	...
	const [goalEvals, setGoalEvals] = useState<GoalEvaluation[]>(
		state.confirmedDraft
			...
			: (aiDraft?.goalEvaluations.map((ge) => ({
					...ge,
					changeReason: "",
				})) ?? []),
	);
	const [overallGrade, setOverallGrade] = useState<EvaluationGrade | "">(
		state.confirmedDraft
			? state.confirmedDraft.overallGrade
			: (aiDraft?.overallGrade ?? ""),
	);
	const [overallRationale, setOverallRationale] = useState(
		state.confirmedDraft
			? state.confirmedDraft.overallRationale
			: (aiDraft?.overallRationale ?? ""),
	);
	const [selfEvalGap, setSelfEvalGap] = useState(
		state.confirmedDraft
			? state.confirmedDraft.selfEvalGap
			: (aiDraft?.selfEvalGap ?? ""),
	);
	...
	const updateGoalEval = (index: number, updates: Partial<GoalEvaluation>) => {
		setGoalEvals((prev) =>
			prev.map((ge, i) => (i === index ? { ...ge, ...updates } : ge)),
		);
	};
	...
	const isValid = (() => {
		...
		for (let i = 0; i < goalEvals.length; i++) {
			if (isGradeChanged(i) && goalEvals[i].changeReason.trim() === "")
				return false;
		}
	...
	return (
		<div>
			...
			<p className="text-xl text-gray-500 mb-8">
				AIドラフトを確認し、必要に応じて修正してください。評価を変更した場合は変更理由の入力が必要です。
			</p>
			...
			<div className="space-y-6 mb-10">
				...
				{goalEvals.map((ge, i) => {
					...
					return (
						<div
							key={ge.goalLabel}
							className={`border rounded-lg p-6 ${changed ? "border-amber-300 bg-amber-50/30" : "border-gray-200 bg-gray-50"}`}
						>
							...
							<div className="mb-4">
								<span className="block text-xl font-medium text-gray-700 mb-2">
									評価
								</span>
							...
							{changed && (
								<div>
									<label
										htmlFor={`change-reason-${i}`}
										className="block text-xl font-medium text-amber-700 mb-2"
									>
