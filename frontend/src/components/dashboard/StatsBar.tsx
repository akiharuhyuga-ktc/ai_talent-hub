...
export function StatsBar({ members }: StatsBarProps) {
	...
	const totalAll = members.reduce(
		(sum, m) => sum + m.projects.reduce((s, p) => s + p.avgPct, 0),
		0,
	);
	const rdRate = totalAll > 0 ? Math.round((totalRd / totalAll) * 100) : 0;
	const actualRate =
		totalAll > 0 ? Math.round(((totalAll - totalRd) / totalAll) * 100) : 0;
		...
		<div className="space-y-4">
			...
			<div className="grid grid-cols-3 gap-4">
				...
				<div className="rounded-radius-xl p-6 bg-white shadow-card">
					...
					<div className="text-[44px] font-bold text-gray-900 leading-none tracking-tight">
						{members.length}
					</div>
				...
				<div className="rounded-radius-xl p-6 bg-gradient-to-br from-brand-600 to-brand-800 shadow-glow-lg">
					<div className="flex items-center justify-between mb-3">
						<span className="text-2xl font-medium text-brand-200">
							R&D稼働率
						</span>
						<FlaskConical size={20} className="text-brand-300" />
					</div>
					<div className="text-[44px] font-bold text-white leading-none tracking-tight">
						{rdRate}
					</div>
			...
			<div className="grid grid-cols-3 gap-4">
				<div className="rounded-radius-xl px-5 py-4 bg-white shadow-card flex items-center gap-3">
					...
					<span className="text-3xl font-bold text-gray-900 ml-auto">
						{flutter}
					</span>
				</div>
				<div className="rounded-radius-xl px-5 py-4 bg-white shadow-card flex items-center gap-3">
