...
export function MemberCard({ member }: MemberCardProps) {
	...
	const rdProject = member.projects.find(
		(p) => p.name.includes("R＆D") || p.name.includes("R&D"),
	);
	const gradient =
	...
	return (
		<div className="bg-white rounded-radius-xl shadow-card p-6 flex gap-5 items-start hover:shadow-card-hover transition-shadow cursor-pointer">
			{/* Avatar */}
			<div
				className={`w-12 h-12 rounded-radius-xl bg-gradient-to-br ${gradient} flex items-center justify-center shrink-0`}
			>
						...
						{member.projects.map((proj) => (
							<div key={proj.name}>
								<div className="flex justify-between items-center mb-1">
									<span className="text-xl text-gray-600 truncate max-w-[70%]">
										{proj.name}
									</span>
									<span className="text-xl font-semibold text-gray-800 ml-2 tabular-nums">
										{proj.avgPct}%
									</span>
								</div>
