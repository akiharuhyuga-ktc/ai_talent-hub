...
export function PolicyStep2BCurrentState({
	onNext,
	onBack,
}: PolicyStep2BCurrentStateProps) {
	...
	const handleNext = () => {
		if (!teamInfo.trim()) {
			setError("「チーム構成・規模」は必須です");
			return;
		}
		if (!techDomains.trim()) {
			setError("「技術領域・担当プロダクト」は必須です");
			return;
		}
		if (!challenges.trim()) {
			setError("「現在の課題」は必須です");
			return;
		}
		if (!strengths.trim()) {
			setError("「組織の強み」は必須です");
			return;
		}
		if (!mission.trim()) {
			setError("「ミッション・役割」は必須です");
			return;
		}
		setError("");
		onNext({
