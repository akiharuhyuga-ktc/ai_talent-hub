...
export function PolicyStep3AThemes({
	onNext,
	onBack,
}: PolicyStep3AThemesProps) {
	...
	const handleNext = () => {
		if (!envChanges.trim()) {
			setError("「事業環境の変化」は必須です");
			return;
		}
		if (!techChanges.trim()) {
			setError("「技術トレンドの変化」は必須です");
			return;
		}
		if (!focusThemes.trim()) {
			setError("「来期の注力テーマ」は必須です");
			return;
		}
		setError("");
		onNext({
