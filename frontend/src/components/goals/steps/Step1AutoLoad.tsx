...
function InfoCard({ label, loaded }: { label: string; loaded: boolean }) {
	return (
		<div className="flex items-center gap-3 p-5 bg-gray-50 rounded-lg border border-gray-200">
			<span
				className={`text-2xl ${loaded ? "text-green-500" : "text-gray-300"}`}
			>
				{loaded ? "✓" : "○"}
			</span>
			<span className="text-xl text-gray-700">{label}</span>
			<span
				className={`ml-auto text-lg px-3 py-1 rounded-full ${loaded ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-400"}`}
			>
				{loaded ? "読込済み" : "未読込"}
			</span>
		</div>
	...
	return (
		<div>
			...
			<p className="text-xl text-gray-500 mb-8">
				目標設定に必要な情報が読み込まれていることを確認してください。
			</p>
			...
			<div className="space-y-4 mb-10">
				...
				<InfoCard
					label="育成基準・評価基準"
					loaded={!!context.evaluationCriteria}
				/>
				<InfoCard
					label={`メンバープロフィール（${context.memberName}）`}
					loaded={!!context.memberProfile}
				/>
