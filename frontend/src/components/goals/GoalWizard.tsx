...
type Action =
	...
	| {
			type: "ADD_REFINEMENT";
			payload: { messages: ChatMessage[]; goals: string; count: number };
	  }
	...
	switch (action.type) {
		...
		case "ADD_REFINEMENT":
			return {
				...
				generatedGoals: action.payload.goals,
			...
			case 2:
				return (
					<Step2ManagerInput
						...
						onNext={(data) =>
							dispatch({ type: "SET_MANAGER_INPUT", payload: data })
						}
			...
			case 3:
				return (
					<Step3MemberInput
						...
						memberName={context.memberName}
						onNext={(data) =>
							dispatch({ type: "SET_MEMBER_INPUT", payload: data })
						}
						onBack={() => dispatch({ type: "PREV_STEP" })}
					/>
				);
			case 4:
				return (
					<Step4PreviousPeriod
						...
						onNext={(data) =>
							dispatch({ type: "SET_PREVIOUS_PERIOD", payload: data })
						}
			...
			case 5:
				return (
					<Step5Diagnosis
						...
						context={context}
						onConfirm={(diag) =>
							dispatch({ type: "CONFIRM_DIAGNOSIS", payload: diag })
						}
						onBack={() => dispatch({ type: "PREV_STEP" })}
					/>
				);
			case 6:
				return (
					<Step6GoalGeneration
						...
						context={context}
						onGenerated={(goals) =>
							dispatch({ type: "SET_GENERATED_GOALS", payload: goals })
						}
			...
			case 7:
				return (
					<Step7Refinement
						state={state}
						context={context}
						onAddRefinement={(msgs, goals, count) =>
							dispatch({
								type: "ADD_REFINEMENT",
								payload: { messages: msgs, goals, count },
							})
						}
						onConfirm={(goals) => {
							dispatch({ type: "SET_FINAL_GOALS", payload: goals });
						}}
