...
function parseGoalFieldContent(raw: string): {
	mainText: string;
	sections: Section[];
} {
				...
				.map((block, i) => {
					...
					if (lines.length > 1) {
						return (
							<ul
								key={i}
								className={`space-y-1.5 list-none pl-0 last:mb-0 ${className}`}
							>
								{lines.map((line, j) => (
				...
				<div className="space-y-5">
					{sections.map((section, i) => {
						...
						return (
							<div
								key={i}
								className={`border-l-[3px] pl-4 ${colorCls.split(" ")[0]}`}
							>
								<p
									className={`text-2xl font-bold mb-2 ${colorCls.split(" ")[1]}`}
								>
									{section.label}
								</p>
