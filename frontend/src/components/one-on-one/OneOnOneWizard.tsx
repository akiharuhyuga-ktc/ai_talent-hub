...
type Action =
	...
	| {
			type: "SET_HEARING";
			payload: { questions: HearingQuestion[]; additionalMemo: string };
	  }
...
function createInitialState(
	...
	const actionReviews: ActionItemReview[] = context.previousActionItems.map(
		(a) => ({
			content: a.content,
			assignee: a.assignee,
			status: "" as const,
...
function reducer(
	...
	switch (action.type) {
		...
		case "SET_HEARING":
			return {
				...state,
				hearingQuestions: action.payload.questions,
				additionalMemo: action.payload.additionalMemo,
				currentStep: 5,
			};
			...
			case 1:
				return (
					<OOStep1ActionReview
						...
						onNext={(reviews) =>
							dispatch({ type: "SET_ACTION_REVIEWS", payload: reviews })
						}
					/>
				);
			case 2:
				return (
					<OOStep2GoalProgress
						...
						onNext={(progress) =>
							dispatch({ type: "SET_GOAL_PROGRESS", payload: progress })
						}
